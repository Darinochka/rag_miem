...
for EMBED in "${EMBEDS[@]}"; do
    ...
    echo "CHUNK_SISE=$CHUNK_SIZE" >> .env.override
    echo "CHUNK_OVERLAP=$CHUNK_OVERLAP" >> .env.override
    echo "RERANKER_MODEL_NAME=$RERANKER_MODEL_NAME" >> .env.override
    ...
    python3 src/utils/get_documents.py --input_csv data/validation/test.csv --output_csv data/out_retriever/CS_500_CO_200_q_rerank/"$PART".csv --rerank
